...
class TFSeq2Seq(SeqToSeq):
    def __init__(self, args, infer=False):
        ...
        with tf.variable_scope('rl'):
            actions = tf.nn.softmax(tf.sigmoid(linear(last_state, 2, True)))
            actions = tf.split(1, 2, actions)

        self.cost = tf.reduce_sum(loss * actions[0]) / args.batch_size / args.seq_length \
                    + tf.reduce_sum(actions[1] * args.action_loss)

        self.action = actions[1]  # do we need to read again
